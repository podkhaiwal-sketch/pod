...
exports.getBetHistory = async (req, res) => {
  try {
    ...
    if (validationResult.error) {
      ...
      const user = await User.findOne({ user_id: req.body.user_id, app_id: req.body.app_id, user_status: 1 }).select('user_id app_id user_status');
      if (user) {
        let page = 1;
        ...
        if (fltDate1 !== '' && tblCode !== undefined) {
          ...
          const formattedDate = formatDate(req.body.flt_date);
          ...
          const result = await PointTable.find(sqlQuery)
            ...
            .skip(offset)
            .limit(itemsPerPage);
            const countQuery = PointTable.countDocuments({ tr_nature: 'TRGAME001', user_id: req.body.user_id});
...
exports.bonus_report_list = async (req, res) => {
  try {
    ...
    if (validationResult.error) {
      ...
      const user = await User.findOne({ user_id: req.body.user_id}).select('user_id commission total_played ref_bonous total_bonus');
      if (user) {
        let page = 1;
        if (req.body.paginate) {
          page = req.body.paginate;
        ...
        const countQuery = BonusReport.countDocuments({user_id: req.body.user_id,tr_nature: 'TRGAME001'});
        const totalCount = await countQuery;
        const comm = await appController.findOne().select('ref_comm');
        const appControllerData = await appController.findOne().select('bonus_min bonus_max');
          const rows = {
            success: "1",
    ...
    if (validationResult.error) {
      ...
      if (user) {
        ...
        if(orderAmount < user.toJSON().ref_bonous){
          var payid = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
          var order_id = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
          const currentDate = moment();
          const date = currentDate.format('YYYY-MM-DD');
          const dateFormat = currentDate.format('DD-MM-YYYY');
          const timeFormat = currentDate.format('hh:mm:ss A');
          const dateTimeFormat = `${dateFormat} (${currentDate.format('ddd')}) ${timeFormat}`;
          ...
          if(orderAmount<=parseInt(user.toJSON().ref_bonous) && orderAmount>=bounsmin && orderAmount<=bounsmax)
          {
            var newcoins = parseInt(user.toJSON.credit) + parseInt(orderAmount);  
            var bonus=parseInt(user.toJSON().ref_bonous)-parseInt(orderAmount);
        ...
        return;
            ...
            const rows = {
              success: "0",
              message: "Please Check Minimum and Maximum Amount Limit"
            };
            res.json(rows);
            return;
          }
